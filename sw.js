...
const CACHE_VERSION = 'v23';                    // <-- bump this on every deploy
...
// Tapping a local notification (morning digest / overdue sweep) should
...
// Real server-sent push — arrives even if the app/browser is fully
// closed, as long as the OS hasn't force-stopped/uninstalled it. The
// Edge Function (send-task-notifications) sends a JSON payload with
// title/body/tag; this just displays it the same way a local
// notification would.
self.addEventListener('push', (event) => {
  let payload = { title: '📋 Tracker', body: 'You have updates waiting.', tag: 'server-push' };
  try {
